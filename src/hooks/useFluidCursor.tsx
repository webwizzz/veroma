...
interface WebGLFormatInfo {
  internalFormat: number;
  format: number;
}

interface WebGLExtensionInfo {
  formatRGBA: WebGLFormatInfo | null;
  formatRG: WebGLFormatInfo | null;
  formatR: WebGLFormatInfo | null;
...
export default function useFluidCursor(
  ...
  configOptions: FluidCursorConfig = {}
) {
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvasEl: HTMLCanvasElement): { gl: WebGL2RenderingContext | WebGLRenderingContext; ext: WebGLExtensionInfo } | null {
      const params = {
        alpha: true,
        depth: false,
        stencil: false,
      ...
      let webgl: WebGLRenderingContext | WebGL2RenderingContext | null = canvasEl.getContext('webgl2', params) as WebGL2RenderingContext | null;
      const isWebGL2 = !!webgl;
      if (!isWebGL2) {
        webgl = (canvasEl.getContext('webgl', params) || canvasEl.getContext('experimental-webgl', params)) as WebGLRenderingContext | null;
      }
      ...
      const halfFloatTexType = isWebGL2 ? (webgl as WebGL2RenderingContext).HALF_FLOAT : halfFloat && halfFloat.HALF_FLOAT_OES;
      let formatRGBA: WebGLFormatInfo | null = null;
      let formatRG: WebGLFormatInfo | null = null;
      let formatR: WebGLFormatInfo | null = null;
    ...
    if (!baseVertexShader || !copyShader || !clearShader || !splatShader || !advectionShader || !divergenceShader || !curlShader || !vorticityShader || !pressureShader || !gradientSubtractShader) {
      console.error('Failed to compile essential shaders.');
      return;
    }
